...
vcpkg_make_configure(
    ...
    OPTIONS
        ...
        # Symbol versioning is an ELF feature. libffi turns it on by default and then emits
        # -Wl,--version-script,libffi.map, which ld.lld rejects outright in COFF/MinGW mode:
        #   lld: error: unknown argument: --version-script
        # The reason it is even attempted is that lld advertises itself as "compatible with GNU
        # linkers", so libtool treats it as GNU ld and assumes GNU ld's feature set. A Windows DLL
        # has no use for symbol versioning anyway -- there is no equivalent in the PE format -- so
        # turning it off costs nothing on this target.
        --enable-symvers=no
        ${options}
)
