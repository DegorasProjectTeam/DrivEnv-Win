...
(
    ...
    # @brief Which toolchain of a dual-environment configuration this run is for, e.g. -Toolchain ucrt.
    #
    # Empty is the single-environment case and must stay the default: a configuration that declares no
    # 'toolchains' is a 2.x configuration, it is read exactly as it stands, and the drive it produces is
    # identical to the one version 2 produced. Nothing below this point knows the difference, because the
    # resolution happens once, here, and hands the rest of the script a configuration with one subsystem and
    # one triplet in it -- the same scalars this file has always read.
    #
    # Normally supplied by Generate-DrivEnv.ps1, which loops the steps over the ids in 'generate'. Given by
    # hand it is how you rebuild one half of a dual drive without touching the half that is already good.
    [string]$Toolchain = "",
...
# --------------------------------------------------------------------
# From here down $Cfg holds ONE toolchain, whatever the file on disk holds. Everything after this line reads
# $Cfg.msys2.target.subsystem and $Cfg.vcpkg.target.triplet as the plain scalars they have always been,
# because after the projection they ARE plain scalars.
#
# ORDER MATTERS: this runs AFTER Test-DrivEnvConfig, never before. The validator is what has already checked
# that the id exists, that no two toolchains collide, and that the projection of every generated toolchain is
# complete -- so by the time the projection happens there is nothing left for it to get wrong.

$generated = Get-DrivEnvGenerateList -Cfg $Cfg

if ([string]::IsNullOrWhiteSpace($Toolchain))
{
    # A dual configuration run without -Toolchain would silently build whichever toolchain the unprojected
    # file happens to resemble, and there is no good candidate: the shared section has no subsystem and no
    # triplet at all. Refusing is the only honest answer.
    if ($generated.Count -gt 0)
    {
        Write-Error ("This configuration generates {0} toolchain(s) -- {1} -- so -Toolchain must say which one." -f $generated.Count, ($generated -join ", "))
        Write-Error "    Run it through Generate-DrivEnv.ps1, which loops the steps over them, or name one here."
        Abort-WithError
    }
}
else
{
    ...
    {
        Write-Error ("-Toolchain '{0}' was given, but this configuration declares no 'toolchains'." -f $Toolchain)
        Abort-WithError
    }

    if ($generated -notcontains $Toolchain)
    {
        Write-Error ("-Toolchain '{0}' is not generated by this configuration. It generates: {1}" -f $Toolchain, ($generated -join ", "))
        Abort-WithError
    }

    $Cfg = Resolve-DrivEnvToolchain -Cfg $Cfg -Id $Toolchain
    Write-Info ("Toolchain '{0}': subsystem '{1}', triplet '{2}'." -f $Toolchain, $Cfg.msys2.target.subsystem, $Cfg.vcpkg.target.triplet)
}
